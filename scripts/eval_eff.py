...
if __name__=="__main__":
    ...
    model = Network(bn_out=(256 // 16, 448 // 16), use_kan=False).cuda()
    
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(model, (7, 3, 256, 448), as_strings=True,
                                                 print_per_layer_stat=False, verbose=True)
    inputs = torch.randn(1, 7, 3, 256, 448)
