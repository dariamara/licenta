...
class Medical(object):
    def __init__(self, length):
        ...
        self.threshold_TP = np.zeros((length, len(self.Thresholds)))
        self.threshold_TN = np.zeros((length, len(self.Thresholds)))
        self.threshold_FP = np.zeros((length, len(self.Thresholds)))
        self.threshold_FN = np.zeros((length, len(self.Thresholds)))
    ...
    def Fmeasure_calu(self, pred, gt, threshold):
        ...
        return RecallFtem, SpecifTem, Dice, IoU, TRP, 1-TRP, FPR, 1-FPR
    ...
    def step(self, pred, gt, idx):
        ...
        threshold_TP = np.zeros(len(self.Thresholds))
        threshold_TN = np.zeros(len(self.Thresholds))
        threshold_FP = np.zeros(len(self.Thresholds))
        threshold_FN = np.zeros(len(self.Thresholds))

        for j, threshold in enumerate(self.Thresholds):
            threshold_Rec[j], threshold_Spe[j], threshold_Dic[j], \
            threshold_Iou[j],  threshold_TP[j], threshold_TN[j], threshold_FP[j], threshold_FN[j] = self.Fmeasure_calu(pred, gt, threshold)
        ...
        self.threshold_TP[idx, :] = threshold_TP
        self.threshold_TN[idx, :] = threshold_TN
        self.threshold_FP[idx, :] = threshold_FP
        self.threshold_FN[idx, :] = threshold_FN
    ...
    def get_results(self):
        ...
        column_TP = np.mean(self.threshold_TP, axis=0)
        column_TN = np.mean(self.threshold_TN, axis=0)
        column_FP = np.mean(self.threshold_FP, axis=0)
        column_FN = np.mean(self.threshold_FN, axis=0)
        ...
        return dict(meanSen=column_Sen, meanSpe=column_Spe, meanDice=column_Dic, meanIoU=column_IoU,
                    ...
                    meanTP=column_TP, meanTN=column_TN, meanFP=column_FP, meanFN=column_FN)