    ...
    for _data_name in opt.data_lst[0]:
        ...
        with open(filename, 'w+') as file_to_write:
            ...
            for _model_name in opt.model_lst[0]:
                ...
                case_list = [c for c in os.listdir(gt_src) if os.path.isdir(os.path.join(gt_src, c))]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='../data/SUN-SEG/TestHardDataset/Seen/GT')
    parser.add_argument(
        '--pred_root', type=str, help='custom your prediction root',
        default='/storage/datasets/pred/') #initial : ../data/Pred/
    parser.add_argument(
    ...
    if opt.check_integrity:
        print("check intgrity")
        for _data_name in opt.data_lst[0]:
            for _model_name in opt.model_lst[0]:
                gt_pth = opt.gt_root
                pred_pth = opt.pred_root
                if not sorted(os.listdir(gt_pth)) == sorted(os.listdir(pred_pth)):
                    print(sorted(os.listdir(gt_pth)))
                    print(sorted(os.listdir(pred_pth)))
