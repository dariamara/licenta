...
class ConvLayer(nn.Module):
    """Double conv block used in U-KAN encoder stages: Conv→BN→ReLU→Conv→BN→ReLU."""
...
class D_ConvLayer(nn.Module):
    """Double conv block used in U-KAN decoder stages: Conv→BN→ReLU→Conv→BN→ReLU with channel reduction."""
    def __init__(self, in_ch, out_ch):
