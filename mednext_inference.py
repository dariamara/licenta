...
@lru_cache(maxsize=1)
def _load_model():
    # use_kan=True: this is the "MedNeXt_KAN" checkpoint. Parameter shapes don't depend on
    # use_kan (the KAN blocks are always constructed), so a mismatch here wouldn't show up
    # as a state_dict load error -- only as wrong outputs -- but this is the variant the
    # weights file corresponds to.
...
def process_frames(input_paths, output_paths):
    """
    Runs PNSNet (MedNeXt+KAN) video-polyp-segmentation over a sequence of consecutive frames.
